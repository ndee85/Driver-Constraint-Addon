...
class CreateDriverConstraint(bpy.types.Operator):
    ...
    def get_action_constraints(self,context):
        ...
        for bone in context.selected_pose_bones:
            for const in bone.constraints:
                if const.action != None and const.action.name not in action_names:
                    action_names.append(const.action.name)
                    ACTIONS.append((const.action.name,const.action.name,const.action.name,"ACTION",i))
                    i += 1
    ...
    action = bpy.props.EnumProperty(name="Action",items=get_actions,description="Choose Action that will be driven by Bone")
    ...
    def create_actions_constraints(self,context):
        if self.action_mode == "ADD_CONSTRAINT":
            ...
            for bone in context.selected_pose_bones:
                for const in bone.constraints:
                    if (const.action != None and const.action.name == self.action_constraint) or (self.action_constraint == "ALL_ACTIONS"):
